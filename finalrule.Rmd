...
data_labels$How.many.years.experience.do.you.have.as.a.practicing.genetic.counselor..round.to.the.nearest.year.. <- factor(data_labels$How.many.years.experience.do.you.have.as.a.practicing.genetic.counselor..round.to.the.nearest.year.., levels = levels, ordered = TRUE)
...
ben<-data%>%subset(select = c(pt_empower:pt_share)) %>% mutate_all(as.numeric) %>% mutate_all(na.omit)
...
harm<-data%>%subset(select = c(pt_reaction_RC:pt_contact_RC)) %>% mutate_all(as.numeric) %>% mutate_all(na.omit)
...
# create harms score
data_raw$harms <- rowSums(data_raw[,c("pt_reaction_RC")], na.rm = TRUE)
